...
class GoLogin(object):
    ...
    def loadExtensions(self):
        ...
        default_ext_id = "hlifkpholllijblknnmbfagnkjneagid"
        ver = extensionsManagerInst.downloadExt(default_ext_id)
        # ver2 = extensionsManagerInst.downloadExt('lanfdkkpgfjfdikkncbnojekcppdebfp')
        installed_ext_paths.append(os.path.join(pathlib.Path.home(), '.gologin', 'extensions', 'chrome-extensions',
                                                default_ext_id + '@' + ver))
        # installed_ext_paths.append(os.path.join(pathlib.Path.home(), '.gologin', 'extensions', 'chrome-extensions',
        #                                         "lanfdkkpgfjfdikkncbnojekcppdebfp" + '@' + ver2))


        for ext in chromeExtensions:
            ver = extensionsManagerInst.downloadExt(ext)
            installed_ext_paths.append(
                os.path.join(pathlib.Path.home(), '.gologin', 'extensions', 'chrome-extensions', ext + '@' + ver))

        pathsToLoad = ','.join(installed_ext_paths)
        return pathsToLoad if installed_ext_paths else None

    # def loadExtensions(self):
    #     profile = self.profile
    #     chromeExtensions = profile.get('chromeExtensions')
    #     extensionsManagerInst = ExtensionsManager()
    #     installed_ext_path = extensionsManagerInst.installExtension("hlifkpholllijblknnmbfagnkjneagid")
    #     pathToExt = ''
