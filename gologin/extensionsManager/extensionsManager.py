...
class ExtensionsManager():
    ...
    def installExtension(self, ext_id):
        print(f"Installing extension with ID: {ext_id}")
        version = self.downloadExt(ext_id)
        if version:
            installed_ext_path = os.path.join(CHROME_EXTENSIONS_PATH, ext_id + '@' + version)
            print(f"Extension installed at: {installed_ext_path}")
            return installed_ext_path
        else:
